...
function statusClass(status: string): string {
  ...
  if (s.includes('reference') || s.includes('checking'))  return styles.sReference;
  ...
  return (
    <div className={`${styles.card} ${styles[sc]}`}>
      ...
      <div className={styles.top}>
        <div className={styles.name}>{applicant.name} <span className={styles.date}>({applicant.date})</span></div>
      ...
      {bad.length > 0 && (
        <div className={styles.reasons}>
          {bad.map((r, i) => (
            <div key={i} className={styles.rBad}>⚠ {r}</div>
          ))}
        </div>
      )}
      ...
      {good.length > 0 && (
        <div className={styles.reasons}>
          {good.map((r, i) => (
            <div key={i} className={styles.rGood}>{r}</div>
          ))}
        </div>
      )}

      {applicant.notes && (
        <div className={styles.note}>{applicant.notes}</div>
      )}
    </div>
  );
}